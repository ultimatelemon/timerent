        ...
        <div class="inline-block min-w-full py-2 align-middle sm:px-6 lg:px-8">
          <table class="min-w-full divide-y divide-gray-300">
            ...
            <tr>
              ...
              <th scope="col" class="whitespace-nowrap px-2 py-3.5 text-left text-sm font-semibold text-gray-900">Waarvan BTW</th>
              <th scope="col" class="whitespace-nowrap px-2 py-3.5 text-left text-sm font-semibold text-gray-900">Betaalstatus</th>
            </tr>
            </thead>
            <tbody class="divide-y divide-gray-200 bg-white">
            <tr v-if="invoices.length > 0" v-for="invoice in invoices" :key="invoices.id" class="even:bg-gray-50 hover:bg-gray-100 hover:cursor-pointer">
              <td class="whitespace-nowrap py-2 pl-4 pr-3 text-sm text-gray-500 sm:pl-0 font-semibold">#{{ invoice.number }}</td>
              <td class="whitespace-nowrap py-2 pl-4 pr-3 text-sm text-gray-500">{{ $filters.humanDate(invoice.created_at) }}</td>
              <td class="whitespace-nowrap py-2 pl-4 pr-3 text-sm text-gray-500">{{ invoice.email }}</td>
              <td class="whitespace-nowrap py-2 pl-4 pr-3 text-sm text-gray-500">{{ $filters.currency(invoice.payment_amount) }}</td>
              <td class="whitespace-nowrap py-2 pl-4 pr-3 text-sm text-gray-500">{{ $filters.currency(invoice.tax_high + invoice.tax_low) }}</td>
              <td class="whitespace-nowrap py-2 pl-4 pr-3 text-sm text-gray-500 label label-success" v-if="invoice.paid_at">Voldaan</td>
              <td class="whitespace-nowrap py-2 pl-4 pr-3 text-sm text-gray-500 label label-danger" v-else>Niet voldaan</td>
            </tr>
