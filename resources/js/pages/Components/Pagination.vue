<template>
  <div class="flex items-center w-full my-8" v-if="pagination">
    <button class="btn btn-lg" :class="current_page > 1 ? 'btn-primary' : 'btn-secondary text-gray-300 cursor-default'" @click="goToPage(current_page - 1)">
      <component :is="ArrowLeftIcon" class="h-5 w-5"></component>
    </button>
    <div class="mx-auto flex items-center">
      <p class="text-sm text-gray-800">Page {{ current_page}} of {{ pagination.last_page }}</p>
      <p class="text-xs text-gray-500 ml-1.5 font-medium italic mr-auto">({{ ((current_page - 1) * pagination.per_page) + 1 }} — {{ Math.min(current_page * pagination.per_page, pagination.total) }} of {{ pagination.total }})</p>
    </div>
    <button class="btn btn-lg" :class="current_page < pagination.last_page ? 'btn-primary' : 'btn-secondary text-gray-300 cursor-default'" @click="goToPage(current_page + 1)">
